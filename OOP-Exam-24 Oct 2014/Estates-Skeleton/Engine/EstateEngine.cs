    ...
    class EstateEngine : IEstateEngine
    {
        ...
        private string ExecuteFindREntsByLocationCommand(string location)
        {
            var rents = this.Offers
                ...
                .OrderBy(o => o.Estate.Name);
            return FormatQueryResults(rents);
        }

        private string ExecuteFindREntsByPriceCommand(string lowerPrice, string upperPrice)
        {

            var rentsOffers = this.Offers
                .Where(o => o.Type == OfferType.Rent);
            List<IRentOffer> castRentsOffers = new List<IRentOffer>();

            foreach (var rentOffer in rentsOffers)
            {
                castRentsOffers.Add((IRentOffer)rentOffer);
            }

            var sortedRentsOffers = castRentsOffers.Where(
                o => o.PricePerMonth >= int.Parse(lowerPrice) && o.PricePerMonth <= int.Parse(upperPrice))
                .OrderBy(o => o.PricePerMonth)
                .ThenBy(o => o.Estate.Name);
               
            return FormatQueryResults(sortedRentsOffers);
        }
